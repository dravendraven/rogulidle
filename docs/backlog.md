...
| C1 | The route becomes a decision: `sideAppetite` splits into risk appetite × greed, `cautela` prices exposure per turn, the dark stops being free, the frontier becomes a real candidate and the refuge gives the bot its first goal meaning "away". Seven pieces, each buildable and watchable alone, plus what was tried and rejected — `docs/project/cautela.md` | READY — start with the split, which ships as a no-op |
