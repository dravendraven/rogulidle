...
| M50 | **The generator learns the second spine** — a floor layout guaranteeing TWO routes hero→hole: disjoint except at the ends, one meaningfully shorter, and every tile classified at generation (short-route / long-route / side) so placement can read the classification: the short route takes the denser mass share, the long one the sparser. The map's own philosophy section ("The owner's target shape", `docs/map-design.md`) is the spec. STRUCTURAL, not tuning: no dial values move, map size stays where it is (growing it is a separate, later value change). Tests prove the topology (two routes, disjointness, length asymmetry, placement asymmetry); the run trace records which route the hero walked, so "does route choice fall out of caution × dangerField for free" — the design's load-bearing bet — becomes measurable the day this lands. `layout-hub.js`'s rings (two ways around by construction) are the starting point | **BUILT, shipped OFF** (`RING_EVERY` 0) — 2026-08-18. 227 tests pass; 12-run hash proves the shipped game is byte-identical. **First reading of the load-bearing bet, and it FAILS as measured**: long-route tile share reads ~55% flat across caution minima/centre/maxima (40 runs each) — caution does not separate the arms. CAVEAT before concluding: the metric counts every tile walked (sweeping, backtracking), not which arc the hero committed to hole-ward; a commit-metric may read differently. Judging the bet needs that metric, or watching |
