...
| | `MAP_LAYOUT` | `digger` | which generator draws the floor. `digger` is ROT's accretion, `hub` is a central room with a ring around it (`src/sim/layout-hub.js`). A CATALOGUE, not a replacement — the thing worth copying from DCSS, see `docs/project/dcss-layouts.md` |
