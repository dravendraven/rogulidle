...
| | `COIN_PILE_PER_FLOOR` / `COIN_PILE_AMOUNT` / `COIN_PILE_ROUTE_GAP` | 1 / 2 / 8 | the explorer's income (2026-08-31): a visible pile per floor, at least ROUTE_GAP walked steps from every tile of the mandatory route (BFS; deeper pockets weighted heavier), credited on contact and paid with the traversal. Floors with no pocket that deep have none. Distance, not room labels, because open themes let a "side" anchor hug the walked path. Chests are untouched: the chest-coin variant traded sustain for coin and fired the opening-deaths wire (0.45 → 0.53 at n=300, with and without compensation) |
