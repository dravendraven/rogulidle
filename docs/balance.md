...
| | `RING_EVERY` | 0 | which floors are dug as a RING — a cycle of rooms, so the floor has two routes to the hole (M50, `map-design.md`'s target shape). Works like `HUB_EVERY`; **0 is OFF, which is the shipped game** |
...
| | `MAP_THEME` | 0 | which shape family draws the floor — an index into `MAP_THEME_LAYOUTS` (padrão / cripta / grade / caverna / anel / central / sorteio). **0 is the shipped game**: the hubEvery/ringEvery dials decide. The catalogue exists to TEST identities in the Lab before the final map design is chosen (M51) |
...
|---|---|---|
| `TURN_BUDGET` | 180 | turns one traversal may spend; running out ends the run. Shown on the HUD as the stamina bar — a real constraint now, not a safety guard. The bot does not read it |
| `COIN_RATE` | 20 | what a completed traversal pays per unit of xp-per-turn; the payout is rounded, so this is also its RESOLUTION — at 10 almost every floor paid 1, 2 or 3. Doubled 2026-08-15 together with the shop's prices (health 1 / shield 2 / dagger 10 / axe 16, `src/ui/shop.js`) and pawa's stairs price (2, `HEROES`), so purchasing power is unchanged and only the step size halved |
