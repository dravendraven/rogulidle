...
| | `COIN_PILE_PER_FLOOR` / `COIN_PILE_AMOUNT` | 1 / 2 | the explorer's income (2026-08-31): one pile per floor, placed OUT OF SIGHT of the whole mandatory route — farther than `VISIBLE_DIST` from every route tile, a bar derived rather than chosen (a walked-steps gap was tried and max-pressa heroes still collected piles their eyes caught from the route; a room filter lied before that). Deeper pockets weigh heavier; floors with no blind pocket have none. Credited on contact, paid with the traversal. Chests untouched: the chest-coin variant traded sustain for coin and fired the deaths wire (0.45 → 0.53, n=300) |
