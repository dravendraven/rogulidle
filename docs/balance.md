...
| | `CHEST_COIN_SHARE` / `CHEST_COIN_AMOUNT` | 0.25 / 1 | of the chests that hold something, how often it is COINS instead of an item, and how many — taken out of the content draw, not added on top, so item income trades for coin income. The second economy (2026-08-31): flat floor income for the explorer against the diver's xp-per-turn rate; sized at half the estimated break-even on purpose, to be judged by the Pressa sweep's coin column |
