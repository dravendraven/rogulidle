...
| | `COIN_CHEST_AMOUNT` | 1 | the explorer's income (2026-08-31): the chest FARTHEST from the mandatory route (walked steps, BFS; no draw consumed) carries these coins on top of its ordinary draw. Hidden in a chest, no magnetism; on top, no sustain lost — the two failures the earlier shapes measured. If the extra income moves the win wires, the counterweight is `COIN_RATE`, never the resources |
...
| `GAME_VERSION` | 1 | the engine's generation, stamped into achievement receipts. Verification only REPLAYS receipts from the current version; older stamps are legacy, trusted on the verification they passed when fresh — so a balance change never un-earns an honest player's pig. **Bump in the same commit as any change that moves how a seed plays** (the selftest anchor re-record is the reminder) |
| `COIN_RATE` | 14 | (20 → 14 on 2026-09-02, the counterweight to the coin chest: at 2 coins on 20 the session game snowballed — 111 clears in 320, a 206-item pile — and 1 coin on 14 is the only pair measured that leaves the session identical to pre-coin) what a completed traversal pays per unit of xp-per-turn; the payout is rounded, so this is also its RESOLUTION — at 10 almost every floor paid 1, 2 or 3. Doubled 2026-08-15 together with the shop's prices and pawa's stairs price (2, `HEROES`), so purchasing power was unchanged and only the step size halved. The shop's own prices live in `src/ui/shop.js`, which states the ruler they come from — they are not dials and are not restated here |
